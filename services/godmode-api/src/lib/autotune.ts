/**
 * AutoTune — контекстно-адаптивный движок параметров LLM.
 *
 * Анализирует сообщение, определяет тип контекста (code/creative/analytical/chat/chaotic)
 * и подбирает оптимальные параметры генерации (temperature, top_p, top_k, penalties).
 *
 * Включает EMA-based обучение: thumbs up/down корректирует профили со временем.
 *
 * Адаптировано из G0DM0D3 AutoTune (elder-plinius/G0DM0D3).
 */
...
function detectContext(msg: string, history: { role: string; content: string }[] = []): {
  type: ContextType; confidence: number; scores: ContextScore[]
} {
  const raw: Record<ContextType, number> = { code: 0, creative: 0, analytical: 0, conversational: 0, chaotic: 0 }
  ...
  if (total === 0) {
    return { type: 'conversational', confidence: 0.5, scores: [{ type: 'conversational', score: 1, pct: 100 }] }
  }
...
interface LearnedDelta { param: keyof TuneParams; delta: number; samples: number }
const learned = new Map<ContextType, LearnedDelta[]>()
const EMA_ALPHA = 0.15

export function recordFeedback(context: ContextType, params: TuneParams, rating: 1 | -1) {
  let deltas = learned.get(context)
  if (!deltas) { deltas = []; learned.set(context, deltas) }

  const base = CONTEXT_PROFILES[context]
  for (const k of Object.keys(base) as (keyof TuneParams)[]) {
    let entry = deltas.find(d => d.param === k)
    if (!entry) { entry = { param: k, delta: 0, samples: 0 }; deltas.push(entry) }
...
export function computeParams(opts: {
  strategy: Strategy
  message: string
  ...
  if (overrides) {
    for (const [k, v] of Object.entries(overrides)) {
      if (v != null) base[k as keyof TuneParams] = v
    }
  }

  const labels: Record<ContextType, string> = {
    code: 'programming', creative: 'creative', analytical: 'analytical',
    conversational: 'chat', chaotic: 'chaotic',
  }
