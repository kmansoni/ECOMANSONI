...
export const MetroMapViewer = memo(function MetroMapViewer({
    ...
    return (
      <div className={cn('flex items-center justify-center p-8', className)}>
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-gray-400">Загрузка схемы метро...</span>
        </div>
      </div>
    );
  }
  ...
  if (!metroCity) {
    return (
      <div className={cn('flex items-center justify-center p-8', className)}>
        <p className="text-sm text-gray-500">Схема метро для «{city}» не найдена</p>
      </div>
    );
  }
          ...
          {metroCity.lines.map(line => (
            <div key={line.id}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-gray-500">{line.stations.length} ст.</span>
              </div>
        ...
        <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-400">
              ~{Math.round(route.totalTimeMinutes)} мин
            </span>
          </div>
            ...
            {route.segments.map((seg, i) => (
              <div key={i} className="flex items-center gap-2 text-xs">
                ...
                <span className="text-gray-500 ml-auto">
                  {seg.stationCount} ст. · {Math.round(seg.durationMinutes)} мин
                </span>
              </div>
            ))}
          </div>
      ...
      {selectedStation && (
        <div className="p-3 rounded-xl bg-gray-800/80 border border-white/10">
          ...
          <div className="flex gap-2 mt-1 text-xs text-gray-400">
            {selectedStation.wheelchairAccessible && <span>♿ Доступна</span>}
            {selectedStation.transferStations.length > 0 && (
              <span>🔄 Пересадки: {selectedStation.transferStations.length}</span>
            )}
          </div>
        </div>
      )}
