...
import { useReelLikes, type ReelLiker } from "@/hooks/useReelLikes";
import { useAuth } from "@/hooks/useAuth";
import { supabase } from "@/integrations/supabase/client";
import { buildProfilePath } from "@/lib/users/profileLinks";
import { cn } from "@/lib/utils";

interface ReelLikesSheetProps {
  reelId: string | null;
  likeCount: number;
  isOpen: boolean;
  onClose: () => void;
...
export function ReelLikesSheet({ reelId, likeCount, isOpen, onClose }: ReelLikesSheetProps) {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { likers, loading, loadingMore, hasMore, error, loadMore } = useReelLikes(
    isOpen ? reelId : null
  );
