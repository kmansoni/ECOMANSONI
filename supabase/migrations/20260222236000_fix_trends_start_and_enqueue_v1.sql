-- Fix: trends run start/enqueue helpers
-- - enqueue_decision_job_v1 had ambiguous "status" reference (OUT param vs column)
-- - start_trend_run_v1 should not fail if decision_jobs schema differs; enqueue is best-effort
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    SELECT dj.job_id, dj.status
      INTO v_job_id, v_status
    FROM public.decision_jobs dj
    WHERE dj.idempotency_key = p_idempotency_key
      AND dj.organization_id = v_org_id
  ...
  -- Best-effort insert. If decision_jobs schema differs, caller should handle exception.
  ...
  RETURNING decision_jobs.job_id, decision_jobs.status
  INTO v_job_id, v_status;
...
BEGIN
  ...
  -- Enqueue is best-effort: worker can still claim via trend_runs if enqueue fails.
  BEGIN
