-- Anti-abuse: ensure compute_user_spam_score_v1 never returns NULL trust_weight
-- Reason: SELECT .. INTO from an empty CTE assigns NULL to target variables.
...
BEGIN
  ...
  -- Apply trust override if present; ensure fallback always keeps a non-NULL value.
  SELECT COALESCE(
    (
