-- ============================================================================
-- FINAL FIX: reels_engine_apply_action ambiguous column references
--
-- Problem: RETURNS TABLE columns (status, message) conflict with table columns
-- Solution: Fully qualify ALL column references in UPDATE/INSERT/SELECT using
--           table aliases, keep signature unchanged for backward compatibility
-- ============================================================================
...
BEGIN
  ...
  -- Use table alias to avoid ambiguity with OUT parameter 'status'.
  ...
  -- Use explicit column list to avoid conflict with OUT parameters.
  ...
  IF v_status = 'accepted' THEN
    ...
    IF p_action_type = 'set_pipeline_suppression' THEN
      ...
      -- Use table alias 's' and qualify all column references.
    ...
    -- Mark journal row as executed. Use table alias 'j' and qualify columns.
  ...
  -- Return using local variables to avoid ambiguity.
  RETURN QUERY
