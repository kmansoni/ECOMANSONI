...
CREATE INDEX IF NOT EXISTS idx_bot_sessions_bot ON public.bot_sessions (bot_id);
CREATE INDEX IF NOT EXISTS idx_bot_sessions_user ON public.bot_sessions (user_id);
CREATE INDEX IF NOT EXISTS idx_bot_sessions_bot_user ON public.bot_sessions (bot_id, user_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_bot_sessions_unique ON public.bot_sessions (bot_id, user_id) WHERE expires_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_bot_handlers_bot ON public.bot_handlers (bot_id);
CREATE INDEX IF NOT EXISTS idx_bot_handlers_active ON public.bot_handlers (bot_id, is_active);
CREATE INDEX IF NOT EXISTS idx_bot_handlers_priority ON public.bot_handlers (bot_id, priority);
...
CREATE INDEX IF NOT EXISTS idx_bot_keyboards_bot ON public.bot_keyboards (bot_id);
...
CREATE INDEX IF NOT EXISTS idx_bot_runs_session ON public.bot_runs (session_id);
CREATE INDEX IF NOT EXISTS idx_bot_runs_created ON public.bot_runs (created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_bot_states_bot ON public.bot_conversation_states (bot_id);
...
CREATE INDEX IF NOT EXISTS idx_bot_topics_bot ON public.bot_topics (bot_id);
-- ============================================================================
-- ROW LEVEL SECURITY
-- ============================================================================
...
-- Policies
DROP POLICY IF EXISTS "Bot owners manage sessions" ON public.bot_sessions;
...
DROP POLICY IF EXISTS "Bot owners manage handlers" ON public.bot_handlers;
CREATE POLICY "Bot owners manage handlers" ON public.bot_handlers
    FOR ALL USING (bot_id IN (SELECT id FROM public.bots WHERE owner_id = auth.uid()));
DROP POLICY IF EXISTS "Bot owners manage keyboards" ON public.bot_keyboards;
CREATE POLICY "Bot owners manage keyboards" ON public.bot_keyboards
    FOR ALL USING (bot_id IN (SELECT id FROM public.bots WHERE owner_id = auth.uid()));
DROP POLICY IF EXISTS "Bot owners view runs" ON public.bot_runs;
CREATE POLICY "Bot owners view runs" ON public.bot_runs
    FOR ALL USING (bot_id IN (SELECT id FROM public.bots WHERE owner_id = auth.uid()));
DROP POLICY IF EXISTS "Bot owners manage states" ON public.bot_conversation_states;
CREATE POLICY "Bot owners manage states" ON public.bot_conversation_states
    FOR ALL USING (bot_id IN (SELECT id FROM public.bots WHERE owner_id = auth.uid()));
DROP POLICY IF EXISTS "Bot owners manage topics" ON public.bot_topics;
...
DROP TRIGGER IF EXISTS bot_handlers_updated_at ON public.bot_handlers;
CREATE TRIGGER bot_handlers_updated_at BEFORE UPDATE ON public.bot_handlers
    FOR EACH ROW EXECUTE FUNCTION public.bot_updated_at_trigger();
DROP TRIGGER IF EXISTS bot_keyboards_updated_at ON public.bot_keyboards;
CREATE TRIGGER bot_keyboards_updated_at BEFORE UPDATE ON public.bot_keyboards
    FOR EACH ROW EXECUTE FUNCTION public.bot_updated_at_trigger();
DROP TRIGGER IF EXISTS bot_conversation_states_updated_at ON public.bot_conversation_states;
CREATE TRIGGER bot_conversation_states_updated_at BEFORE UPDATE ON public.bot_conversation_states
    FOR EACH ROW EXECUTE FUNCTION public.bot_updated_at_trigger();
DROP TRIGGER IF EXISTS bot_topics_updated_at ON public.bot_topics;
