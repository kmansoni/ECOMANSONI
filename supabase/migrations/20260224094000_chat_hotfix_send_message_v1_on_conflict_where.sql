-- Hotfix: make send_message_v1 ON CONFLICT compatible with partial unique index
-- Partial unique index: idx_messages_conv_sender_client_msg WHERE client_msg_id IS NOT NULL
-- Fix: specify matching WHERE clause in ON CONFLICT inference.
