-- Silent messages: suppress recipient-side sound/vibration alerts.
-- Idempotent: ADD COLUMN IF NOT EXISTS + backfill NULLs.

ALTER TABLE public.messages
  ADD COLUMN IF NOT EXISTS is_silent BOOLEAN;

UPDATE public.messages
SET is_silent = false
WHERE is_silent IS NULL;

ALTER TABLE public.messages
  ALTER COLUMN is_silent SET DEFAULT false;

ALTER TABLE public.messages
  ALTER COLUMN is_silent SET NOT NULL;

COMMENT ON COLUMN public.messages.is_silent IS
  'Suppresses recipient-side sound/vibration alerts for this message.';

-- Recreate send_message_v1 with 4th parameter: is_silent.
-- Drop old overloads to avoid ambiguity.
DROP FUNCTION IF EXISTS public.send_message_v1(UUID, UUID, TEXT, BOOLEAN);
