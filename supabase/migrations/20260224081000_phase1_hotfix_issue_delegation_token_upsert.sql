-- Phase 1 hotfix: make issue_delegation_token_v1 idempotent
-- Root cause: unique partial index allows only one active delegation per (tenant_id,user_id,service_id).
-- Fix: reuse/update existing active delegation instead of inserting a new row.
...
BEGIN
  ...
  IF v_delegation_id IS NULL THEN
    ...
    RETURNING delegations.delegation_id INTO v_delegation_id;
  ELSE
    UPDATE delegations
       SET scopes = p_scopes,
