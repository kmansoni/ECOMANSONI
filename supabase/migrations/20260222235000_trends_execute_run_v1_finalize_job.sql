-- Trends: finalize job + clear claim lease (v1)
-- Replaces execute_trend_run_v1 to:
--  - clear trend_runs claim fields
--  - best-effort mark decision_jobs as completed/failed when decision_job_id is present
...
BEGIN
  ...
  -- Best-effort: mark decision job completed.
  BEGIN
    UPDATE public.decision_jobs
       SET status = 'completed',
           updated_at = now(),
           result_snapshot_id = v_snapshot_id,
           assigned_worker_id = COALESCE(v_run.claimed_by, 'trends-worker')
     WHERE job_id = v_run.decision_job_id;
  EXCEPTION WHEN OTHERS THEN
    -- decision_jobs schema may vary in older installs; ignore.
    NULL;
  END;
