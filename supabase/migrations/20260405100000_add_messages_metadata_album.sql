-- Добавляем metadata JSONB колонку для messages (альбомы, self_destruct, etc.)
ALTER TABLE public.messages
  ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT NULL;

-- Индекс для быстрого поиска по album_id внутри metadata
CREATE INDEX IF NOT EXISTS idx_messages_metadata_album_id
  ON public.messages USING BTREE (((metadata->>'album_id')))
  WHERE metadata IS NOT NULL AND metadata ? 'album_id';

-- Обновляем send_message_v1: извлекаем metadata из body envelope
...
BEGIN
  ...
  IF payload IS NOT NULL THEN
    ...
    -- Извлекаем metadata из envelope (album_id, self_destruct, etc.)
    ...
    ELSIF kind = 'media' THEN
      ...
      IF final_content = '' THEN
        final_content := '📎';
      END IF;
      IF final_media_type NOT IN ('image','video','voice','video_circle','document') THEN
