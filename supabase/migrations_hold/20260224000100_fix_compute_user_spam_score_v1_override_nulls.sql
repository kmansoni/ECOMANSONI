-- Anti-abuse: fix compute_user_spam_score_v1 override selection nulling trust_weight
-- Reason: SELECT ... INTO from an empty result set sets variables to NULL.
...
BEGIN
  ...
  -- Apply most recent active override if present (without nulling on no-row)
  SELECT COALESCE(
    (
