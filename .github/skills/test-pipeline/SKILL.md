...
### Unit тесты
- [ ] Тест 4: [описание] — regression / previous bug if applicable
...
6. **Readable assertions**: из текста теста и assertion должно быть сразу понятно, что сломалось
7. **Минимальный набор путей**: happy path + error path обязательны даже для маленького модуля
...
## Качество тестового текста

- Assertion message и структура теста должны помогать дебагу, а не просто давать boolean-pass
- Если тест ловит баг, из его имени должно быть видно какой именно инвариант защищён
- Если модуль критичен, добавь хотя бы один тест на отказоустойчивость или негативный сценарий, а не только happy path
