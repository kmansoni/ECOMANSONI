...
Для КАЖДОГО значительного архитектурного решения создай ADR в `docs/architecture/decisions/`:

**Файл:** `docs/architecture/decisions/ADR-{NNN}-{slug}.md`
...
## Контекст
{Почему решение необходимо, какая проблема}
...
## Метрики успеха
{Как измерить что решение правильное}
```

**Когда создавать ADR:**
- Новая таблица / модель данных
- Выбор библиотеки / фреймворка
- Изменение архитектуры (SFU → MCU, REST → WebSocket)
- Стратегия кеширования / индексации
- Breaking change в API

### 2.6. Диаграммы

Обязательные типы диаграмм:

| Тип | Формат | Когда |
|---|---|---|
| **System Context (C4 Level 1)** | Mermaid C4Context | Новый модуль — показать внешние взаимодействия |
| **Container (C4 Level 2)** | Mermaid C4Container | Архитектура модуля — показать Frontend/Backend/DB |
| **Sequence** | Mermaid sequenceDiagram | API flow — показать последовательность вызовов |
| **ER (Entity-Relationship)** | Mermaid erDiagram | Модель данных — показать таблицы и связи |
| **State Machine** | Mermaid stateDiagram | Заказ, поездка, звонок — показать переходы состояний |
