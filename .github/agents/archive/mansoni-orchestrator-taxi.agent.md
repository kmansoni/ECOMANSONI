...
description: "ARCHIVED - Legacy agent definition. Do not select for new tasks. Оркестратор такси. Заказы, маршруты, водители, real-time трекинг, тарифы, диспетчеризация. Use when: такси, заказ поездки, водитель, маршрут, геолокация, тариф, диспетчер, real-time трекинг, Uber, Bolt аналог."
tools:
  - read_file
  - list_dir
  - file_search
