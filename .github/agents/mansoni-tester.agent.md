---
name: mansoni-tester
description: "Тестер Mansoni. Автономное браузерное тестирование через Playwright MCP. 8-фазный протокол: Smoke → Navigation → Interactive → Functional → Security → Performance → Responsive → A11y. Полное покрытие всех функций всех модулей. Видит консоль, баги, сеть в реальном времени. Use when: тестировать UI, проверить фичу в браузере, найти баги, smoke test, real user testing."
tools:
